...
public class PaymentResourceIT {
    ...
    public static Payment createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, ShippingAddress.class).isEmpty()) {
            shippingAddress = ShippingAddressResourceIT.createEntity(em);
            em.persist(shippingAddress);
            em.flush();
        } else {
            shippingAddress = TestUtil.findAll(em, ShippingAddress.class).get(0);
        }
    ...
    public static Payment createUpdatedEntity(EntityManager em) {
        ...
        ShippingAddress shippingAddress;
        if (TestUtil.findAll(em, ShippingAddress.class).isEmpty()) {
            shippingAddress = ShippingAddressResourceIT.createUpdatedEntity(em);
            em.persist(shippingAddress);
            em.flush();
        } else {
            shippingAddress = TestUtil.findAll(em, ShippingAddress.class).get(0);
    ...
    @Test
    @Transactional
    public void getAllPaymentsByShippingAddressIsEqualToSomething() throws Exception {
        // Get already existing entity
        ShippingAddress shippingAddress = payment.getShippingAddress();
        paymentRepository.saveAndFlush(payment);
        Long shippingAddressId = shippingAddress.getId();

        // Get all the paymentList where shippingAddress equals to shippingAddressId
        defaultPaymentShouldBeFound("shippingAddressId.equals=" + shippingAddressId);

        // Get all the paymentList where shippingAddress equals to shippingAddressId + 1
        defaultPaymentShouldNotBeFound("shippingAddressId.equals=" + (shippingAddressId + 1));
    }
